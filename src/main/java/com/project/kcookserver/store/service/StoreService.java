package com.project.kcookserver.store.service;

import static com.project.kcookserver.configure.entity.Status.VALID;
import static com.project.kcookserver.configure.response.exception.CustomExceptionStatus.ALREADY_CREATED_STORE;
import static com.project.kcookserver.configure.response.exception.CustomExceptionStatus.STORE_NOT_FOUND;
...
import com.project.kcookserver.store.repository.OrderDeadlineRepository;
...
public class StoreService {
	...
	private final OrderDeadlineRepository orderDeadlineRepository;
	...
	public Long createStoreByAccount(CustomUserDetails customUserDetails, CreateStoreReq dto) {
		...
		if (optional.isPresent()) {
			throw new CustomException(ALREADY_CREATED_STORE);
		}
	...
	@Transactional
	public void updateOrderDeadline(UpdateOrderDeadlineReq updateOrderDeadlineReq) {
		Store store = storeRepository.findById(updateOrderDeadlineReq.getStoreId()).orElseThrow(() -> new CustomException(STORE_NOT_FOUND));
		OrderDeadline orderDeadline = orderDeadlineRepository.findByStore(store);
		if (orderDeadline != null) {
			orderDeadline.updateDeadLine(updateOrderDeadlineReq.getMondayDeadline(), updateOrderDeadlineReq.getTuesdayDeadline(), updateOrderDeadlineReq.getWednesdayDeadline(), updateOrderDeadlineReq.getThursdayDeadline(), updateOrderDeadlineReq.getFridayDeadline(),
				updateOrderDeadlineReq.getSaturdayDeadline(), updateOrderDeadlineReq.getSundayDeadline());
			return;
		}

		orderDeadline = new OrderDeadline(updateOrderDeadlineReq.getMondayDeadline(), updateOrderDeadlineReq.getTuesdayDeadline(), updateOrderDeadlineReq.getWednesdayDeadline(), updateOrderDeadlineReq.getThursdayDeadline(), updateOrderDeadlineReq.getFridayDeadline(),
			updateOrderDeadlineReq.getSaturdayDeadline(), updateOrderDeadlineReq.getSundayDeadline()).setStore(store);

		orderDeadlineRepository.save(orderDeadline);
	}
}